...
def shortest_distance_between_points2(a, b, expansion, expanded_rows, expanded_columns):
    ...
    columns_between = len(
        [column for column in expanded_columns if low_column < column < high_column]
    )

    return (
        abs(a[0] - b[0])
        + abs(a[1] - b[1])
        + ((expansion - 1) * (rows_between + columns_between))
    )
...
def part1(data):
    ...
    return sum(
        [
            shortest_distance_between_points(x, y)
...
def part2(data, expansion=2):
    ...
    return sum(
        [
            shortest_distance_between_points2(
                x, y, expansion, no_galaxy_rows, no_galaxy_columns
            )
            for x, y in itertools.combinations(galaxies, 2)
        ]
    )
