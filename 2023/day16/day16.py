...
class Grid:
    ...
    def out_of_bounds(self, beam):
        return (
            beam.row < 0
            or beam.row > self.height - 1
            or beam.column < 0
            or beam.column > self.width - 1
        )
        ...
        while len(self.beams) > 0:
            ...
            for beam in self.beams:
                ...
                if (beam.position(), beam.direction) in self.mirrors_triggered:
                    ...
                    if (
                        beam.direction in ["left", "right", "up", "down"]
                        and beam.direction != original_direction
                    ):
                        self.mirrors_triggered.add(
                            (beam.position(), original_direction)
                        )
