...
class Monkey:
    ...
    def inspect_items(self, monkeys):
        for item in list(self.items):  # iterate over a copy of the list so we can manipulate the list
            ...
            if self.operation == "*":
                item.worry_level = operand1 * operand2
                print(f"    Worry level is multiplied by {operand2} to {item.worry_level}.")
            ...
            if item.worry_level % self.divisible_test == 0:
                ...
                print(f"    Current worry level is not divisible by {self.divisible_test}.")

            print(f"    Item with worry level {item.worry_level} is thrown to monkey {target}.")
...
class Game:
    def __init__(self, input_filename, rounds=1):
        self.monkeys = load_monkeys(input_filename)
        report(self.monkeys)
    ...
    def inspection_count_report(self):
        for monkey_number, monkey in enumerate(self.monkeys):
            print(f"Monkey {monkey_number} inspected items {monkey.items_inspected} times.")
